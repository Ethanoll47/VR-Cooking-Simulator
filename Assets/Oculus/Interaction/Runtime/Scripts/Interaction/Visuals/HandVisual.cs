...
namespace Oculus.Interaction
{
    public class HandVisual : MonoBehaviour, IHandVisual
    {
        ...
        public Vector3 oriHandPosition;
        ...
        public void UpdateSkeleton()
        {
            ...
            if (_updateRootPose)
            {
                ...
                {
                   //added: woan ning
                    oriHandPosition = handRootPose.position;                    
                    isHandGrabbed = false;
                    //to check on all grabbable item if any of it is grabbed
                    Grabbable[] grabbableList = FindObjectsOfType<Grabbable>();  
                    foreach (Grabbable g in grabbableList)
                    {
                        if (g.isGrabbed && g.includeInExperiment)
                        {
                            isHandGrabbed = true;  //to indicate object is grabbed and is include in CD ratio experiment
                            break;
                        } 
                    }
                    ...
                    if (isHandGrabbed)
                    {
                        //added below to alter the hand visual position
                        _root.position = newHandPosition;            
